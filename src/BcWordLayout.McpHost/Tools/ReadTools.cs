...
public static class ReadTools
{
    ...
    [Description("Validate a BC Word layout. level='quick' runs structural + binding checks (OpenXML "
                 ...
                 + "resolution, repeater shape, attachedTemplate warning, and a repeater-downgraded ERROR "
                 + "when a control's alias names a whole data item but the control is not a repeating "
                 + "section - the wreckage a Word save leaves when it converts a repeater to a plain "
                 + "rich-text control, which makes the table render one row instead of one per record). "
                 + "One structural complaint is reported as a WARNING rather than an error: w:rPr/w:w=0, the "
                 + "character scaling Word writes into a content control's placeholder run, which renders "
                 + "correctly in Word and BC and cannot be edited by any tool here. level='full' additionally runs a real dry-run merge (sample "
