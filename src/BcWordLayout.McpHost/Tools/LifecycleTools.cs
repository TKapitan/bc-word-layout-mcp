...
public static class LifecycleTools
{
    ...
    [Description("Create a NEW blank BC Word report layout .docx from a report schema, ready to be edited "
                 ...
                 + "in Word and in Business Central instead of each renderer's own application default (a "
                 + "template keeps its own headers/footers and styles/theme untouched - templatePath is the "
                 + "way to pin custom branding). Its own post-build "
    ...
    [Description("Update a layout's BC dataset custom XML part to a NEW schema in place - e.g. after the AL "
                 ...
                 + "always yields an empty list. NAMESPACE RE-POINT: every BC-namespaced binding's "
                 + "w:prefixMappings URI and every control's w:tag are rewritten to the new identity - "
                 + "ALWAYS, not only when the report's own name/id changed (namespaceChanged), because a "
                 + "binding can name a THIRD namespace: a superseded namespace of the same report, or another "
                 + "report's entirely, e.g. after a control was copied between layouts. Business Central "
                 + "rejects such a layout at upload with one InvalidPrefixMapping per offending binding "
                 + "(sandbox-verified), so re-pointing them is the only state BC accepts and the repaired "
                 + "bindings are reported as repointedForeignBindings (with the namespace each one used to "
                 + "name). A binding whose prefixMappings names no BC namespace at all is left untouched. The "
                 + "XPath element-name steps themselves are never rewritten - that is the 'remap where "
                 + "element names match' - so a foreign binding whose XPath ALSO names a column this report "
                 + "lacks is re-pointed AND reported as an orphan. STRUCTURAL GATE ONLY: like every mutating tool, the write is "
