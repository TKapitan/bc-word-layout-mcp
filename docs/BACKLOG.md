...
|---|---|---|
| B41 | **`refresh_xml_part` cannot repair a foreign-namespace binding** | `LayoutRefresher.ReplaceNamespace` rewrites a binding's `w:prefixMappings` only when it already contains the layout's CURRENT namespace, so a binding orphaned onto a different report's namespace (the shape the `binding-namespace` warning surfaces — 20 in `PaymentPracticeByPeriod.docx`, 38 in `SubcontractorDispatchList.docx`) survives a refresh untouched. Today's only repair is `remove_control` + `insert_field`, which is what the warning text tells the user. Decide whether refresh should re-point EVERY BC-namespaced binding to the new namespace (it would fix these layouts in one call, and a foreign-namespace binding is broken either way) or stay conservative and report them in `RefreshResult` as a new orphan category. Needs its own tests either way — this is a semantic change to a tool that already has a documented contract. |
