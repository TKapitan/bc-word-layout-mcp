...
| Font | **[Manual]** | A layout that PINS its fonts renders the same either side: the stock corpus layouts resolve `minorHAnsi` to Calibri through their theme part, and their sandbox renders matched the mock (2026-08-01). A layout that names NO font does not: `create_layout` emits no `styles.xml` and no theme, so Word and BC each apply their own default and the two disagree (`docs/BACKLOG.md` B45). Check which case you are in before reading anything into a font difference. |
...
the font on the one layout that names no font (B45), and amounts where BC's export carries a
`decimalformatter` nothing applies yet (B43).
