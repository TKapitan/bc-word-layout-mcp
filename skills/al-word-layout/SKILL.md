...
Every mutating tool — all eighteen of them, from `insert_field` through the table-structure tools to
...
| Tool | Key params (defaults) | Returns | Reach for it when… |
|---|---|---|---|
| `validate_layout` | `layoutPath`, `level` = `"quick"` \| `"full"` (default `"quick"`) | `level`, `passed`, `errorCount`, `warningCount`, `findings[]` (`check`, `severity`, `message`, `location`). `quick`: OpenXML validity, single BC XML part, `storeItemID` match, XPath resolution, repeater shape/location, `attachedTemplate` warning, dangling `w:tblStyle` reference warning (a table naming a style the layout's styles part does not define — the reference silently does nothing), `compatibility-mode` warning (the layout CONTAINS repeaters but declares a Word compatibility mode below 15 — a Word save would convert them to plain rich-text controls and drop their bindings; blank builds declare 15, so this fires for a `templatePath` shell or a layout authored before that scaffold). `repeater-downgraded` **error** (a control whose alias names a whole data item but which is not a repeating section — what a Word save leaves behind when it converts a repeater to a plain rich-text control; the row then renders once instead of once per record). One structural complaint is deliberately a **warning**, not an error: `w:rPr/w:w=0`, the character scaling Word writes into a content control's placeholder run — it renders correctly in Word and BC and no tool here can edit run properties. `full`: everything `quick` does, plus a real dry-run merge (sample data + repeater expansion) against a throwaway copy, surfacing every merge warning as a finding. | `quick` liberally (cheap, run it constantly); `full` before treating the loop as done — it's the only level that proves every binding actually resolves. |
...
| `insert_subtotal_row` | `layoutPath`, `parentControlId` (the repeater's `controlId`), `cells` (same DSL as `insert_table_row`: `-` spacer, **FULL dataset paths** — the corpus subtotal binds a sibling non-repeating `Subtotals` item and `Labels` captions; `a+b` chaining, `N:` spans; spans must sum to the parent table's `columnCount`), `alignments`=`null`, `bold`=`null`, `fontSizePoints`=`null` | Same shape as `insert_field`, but `controlId` is `0` (the row is a plain `w:tr`; its bound cells' ids are in `summary`) | A STATIC per-group row appended at the END of a repeater's item — the stock group-subtotal shape (SalespersonCommission): renders once per PARENT row, after the nested detail rows, never per detail row (contrast `insert_table_row`, which renders once for the whole table). Repeated calls stack: spacer row (`cells="9:-"`) first, then the bold subtotal row. The group HEADER needs no extra call — the repeater's own line row is it. |
| `insert_repeater_table` | `layoutPath`, `dataItem` (repeating, non-system data item), `columns` (comma-separated leaf column names, in order), `locationType` + its required params, `headerFromLabels`=`true` (bind header cells to `*Lbl` columns when found, else humanized static text), `tableStyle`=`null` (Word table style name, e.g. `"TableGrid"`), `columnWidths`=`null` (comma-separated twips, one per column — **binding**: emitted with `w:tblLayout="fixed"` + a `w:tblW` total, so Word renders the grid as specified rather than autofitting it to the merged content), `columnAlignments`=`null` (comma-separated `left`/`center`/`right`, one per column — applied to the header cell AND the data cell, the way real BC line tables right-align a numeric column as a whole), `layoutPart`=`"body"` **(only supported value in v1)**, `partName`=`null` (unused — always omit) | Same shape as `insert_field` plus `columnCount` | The flagship tool for adding a new line-items-style table: builds a header row (label controls or humanized static text) + one data row wrapped in a repeating-section control, one bound field per column. **Body only** — passing `header`/`footer` is rejected with `invalid_argument`. |
...
| Tool | Key params (defaults) | Returns | Reach for it when… |
|---|---|---|---|
| `create_layout` | `schemaSource` (existing `.docx` layout **or** schema `.xml`), `outputPath`, `templatePath`=`null` (an **unbound** branded/styled shell — headers/footers/logo/fonts/styles, NOT a full BC layout with its own bound controls), `headingText`=`null` (the heading paragraph a BLANK build's body starts with — omit for the report's own name, pass a human document title when authoring from scratch, or `""` for no heading; ignored when a template's body already has content) | `outputPath`, report identity, `storeItemId`, `usedTemplate`, `replacedExistingBcPart`, `quickValidation` | Starting a brand-new layout, optionally over a branded template — §1 ("Where the schema comes from") covers how to obtain the `schemaSource` artifact when none exists yet. Always ships exactly one BC custom XML part (fresh `storeItemID`) plus the glossary part the `insert_*` placeholders depend on. A BLANK (non-template) layout also ships empty header/footer parts wired into its page setup, **a default styles part pinning its typography** (Calibri 11 pt `docDefaults` plus the standard `Normal`/`TableGrid` definitions), so it renders with the same font in Word and in Business Central, **and a settings part declaring `compatibilityMode` 15** — the mode every stock layout declares and the one in which repeating-section controls exist, so a Word round trip cannot silently convert its repeaters to plain rich-text controls — a template keeps its own headers/footers, styles/theme and settings untouched (`validate_layout`'s `compatibility-mode` warning covers the shell that leaves the layout below 15). `templatePath` MUST be unbound: if it already carries a BC part **and** bound controls that would go stale against the fresh `storeItemID`, the call fails outright with `template_not_unbound` (nothing is written) instead of silently shipping a broken layout — see §5. A template whose BC part has zero bound controls of its own still succeeds. |
| `refresh_xml_part` | `layoutPath`, `newSchemaSource` (existing `.docx` **or** schema `.xml`) | Old/new report identity, `namespaceChanged`, `remappedCount`, `orphanedBindings[]` (`alias`, `xpath`, `part`), `repointedForeignBindings[]` (`alias`, `xpath`, `part`, `previousNamespace`), `newUnboundFields[]`, `quickValidation` | After the AL report dataset changes. Replaces the dataset part's content **in place** (keeps the same `storeItemID`, so every existing binding still links to the same part) and reclassifies every existing binding by element name — matches are remapped/kept, non-matches are reported as `orphanedBindings` and **left in place** (this tool never deletes or rebinds anything itself — that's your call via `remove_control`/`insert_field`/`insert_label`). It DOES re-point namespaces: every BC-namespaced binding's `prefixMappings` is rewritten to the layout's namespace, always — including a binding left naming a **foreign** report's namespace, which BC rejects at upload (`InvalidPrefixMapping`) and which nothing else here can repair; those are reported as `repointedForeignBindings`. A non-zero `errorCount` here from `xpath-resolves` findings is the *expected* corroboration of the orphan report, not a failed refresh. |
...
- Repeaters nested **one or more levels deep**: reading, validating, merging, and previewing a
  ...
  repeater per call, `insert_repeater_row` adds a nested DETAIL-ROW repeater inside an existing
  repeater's item — the standard BC per-line-detail shape — and `insert_subtotal_row` appends the
  static per-group spacer/subtotal rows that close each group in Microsoft's grouped list reports.
...
- **After any AL dataset change, call `refresh_xml_part`, then re-validate** (ideally `level=full`) —
  ...
  you. It DOES repair one class on its own: a binding naming a foreign report's namespace, reported
  back as `repointedForeignBindings`, because BC refuses to upload a layout carrying one.
- **Don't rebuild controls by hand to fix a `binding-namespace` error.** One `refresh_xml_part`
  against the report's current schema re-points every such binding — including on a layout whose
  namespace is otherwise unchanged. Rebuilding is only needed when the same control is ALSO in
  `orphanedBindings` (its XPath names a column this report does not have).
